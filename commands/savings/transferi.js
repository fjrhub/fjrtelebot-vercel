...
========================= */
const OPTIONS = {
  akun: ["Wallet", "Dana", "Seabank", "Bank", "Binance", "Fjlsaldo", "Gopay", "Jago"],
};
...
export default {
  ...
  async execute(ctx) {
    ...
    let withAdmin = args.includes("-admin");
    if (withAdmin) {
    ...
    // Parsing argumen: Mendukung 3 argumen (normal) atau 4 argumen (ada selisih)
    if (args.length === 4) {
      [asalStr, tujuanStr, jumlahKirimStr, jumlahTerimaStr] = args;
      withAdmin = true; // Otomatis aktif jika ada 4 argumen (kirim & terima)
    } else if (args.length === 3) {
      [asalStr, tujuanStr, jumlahKirimStr] = args;
      jumlahTerimaStr = null;
    } else {
      return ctx.reply(
        "❌ **Format salah.**\n\n" +
        "**Cara Penggunaan:**\n" +
